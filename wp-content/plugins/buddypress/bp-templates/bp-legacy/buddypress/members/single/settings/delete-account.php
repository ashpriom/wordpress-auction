...
<div id="message" class="info">
	...
	<?php	
	$userID = bp_displayed_user_id(); echo "Buddypress:"; echo $userID;
	...
	if ($userActivityCount != 0){ ?>
		<style> #account-delete-form .submit, #account-delete-form label{ display:none !important;}
		#buddypress div#message.info p{background: lightcoral !important;} </style>
	<?php }
	?>
	...
	<?php if($userActivityCount != 0){ ?>
		...
		<p><?php _e( 'User has current bid active. The account can be deleted only after the auction is closed.', 'buddypress' ); ?></p>
...
<form action="<?php echo bp_displayed_user_domain() . bp_get_settings_slug() . '/delete-account'; ?>" name="account-delete-form" id="account-delete-form" class="standard-form" method="post">
	...
	<div class="submit">
		<?php $userID = $bp->displayed_user->id; echo $userID; ?>
